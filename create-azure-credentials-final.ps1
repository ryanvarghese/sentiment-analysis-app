...
$spName = "github-actions-sentiment-$(Get-Date -Format 'yyyyMMdd-HHmm')"

Write-Host "Service Principal Name: $spName" -ForegroundColor Cyan

# Try with just the app service scope
$creds = az ad sp create-for-rbac --name $spName --role contributor --scopes "/subscriptions/$($sub.id)/resourceGroups/rg-sentimentanalyzer-proj1-dev-001/providers/Microsoft.Web/sites/sentiment-analysis-ryan-20241215" --sdk-auth
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "`nFailed to create service principal. Trying resource group scope..." -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "`nStill failed. You may need admin permissions." -ForegroundColor Red
        Write-Host "`nAlternative: Update the publish profile secret instead:" -ForegroundColor Yellow
        Write-Host "1. Use the publish profile from the previous script" -ForegroundColor Cyan
        Write-Host "2. Update AZUREAPPSERVICE_PUBLISHPROFILE secret" -ForegroundColor Cyan
    }
}
